...
Новый раздел, отображающий историю изменений тегов для спортивных объектов.
...
**Функциональность:**
- **Связь по Email:** Кнопка для отправки сообщения разработчику через почтовый клиент.
- **Связь через Telegram:** Кнопка для быстрого перехода в чат Telegram с разработчиком.
- **Безопасность данных:** Контактная информация разработчика хранится в .env файле и не включается в публичный репозиторий.
- **Интуитивный интерфейс:** Крупные, заметные кнопки с иконками для различных способов связи.
...
- **Добавлена информация о новых разделах:** Теперь включает описание разделов "История изменений" и "Поддержка".
...
### Управление переменными окружения (EnvConfig)

Новый компонент для загрузки и управления переменными окружения.

**Функциональность:**
- **Загрузка .env файла:** Автоматическая загрузка переменных окружения при запуске приложения.
- **Безопасное хранение данных:** Хранение чувствительной информации вне кода приложения.
- **Доступ к переменным:** Централизованный доступ к переменным окружения через статические методы.

**Технические особенности:**
- Использует библиотеку `flutter_dotenv` для работы с .env файлами
- Предоставляет типизированный доступ к переменным окружения
- Обрабатывает случаи, когда переменная не найдена
- Загружается в начале запуска приложения для доступности данных во всех компонентах
...
### 8. Добавление разделов "История" и "Поддержка"
...
- **Раздел "История":**
  ...
  - Показывает кто (email модератора), когда и какие изменения внес
...
### Добавление обучающего окна для новых пользователей
...
### Улучшение индикатора загрузки деталей объекта
- **Функциональность:** Заменен способ отображения процесса загрузки деталей объекта при нажатии на плейсмарк.
- **Интерфейс:**
  ...
  - Под индикатором добавлена подпись "Загрузка данных..." белым жирным шрифтом
- **Техническая реализация:**
...
- **Преимущества перед предыдущей реализацией:**
  - Более заметная индикация загрузки, предотвращающая множественные нажатия
  - Четкое указание пользователю о процессе загрузки данных
  - Универсальный подход, работающий на всех размерах экрана
- **Файл:** `lib/scenes/map_screen.dart`
